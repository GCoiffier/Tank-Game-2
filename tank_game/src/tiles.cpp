...
int encode_neighbours(Grid<anax::Entity> &map, int i, int j) {
    const int n = map.nb_lines();
    const int m = map.nb_cols();
    ...
    if (j == 0) { // Top border
        if (i == 0) { // Upper-left corner
            code += map(i + 1, j).isValid() ? 1 << RIGHT : 0;
            code += map(i, j + 1).isValid() ? 1 << DOWN : 0;
        }
        else if (i == n-1) { // Upper-right
            code += map(i, j - 1).isValid() ? 1 << LEFT : 0;
            code += map(i - 1, j).isValid() ? 1 << DOWN : 0;
        }
        else {
            code += map(i - 1, j).isValid() ? 1<<LEFT : 0;
            code += map(i + 1, j).isValid() ? 1<<RIGHT : 0;
            code += map(i, j + 1).isValid() ? 1<<DOWN : 0;
        }
